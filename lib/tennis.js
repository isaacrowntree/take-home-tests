...
const match = (name1, name2) => {
  ...
  const hasWonByMargin = (score1, score2) => {
    return Math.abs(score1 - score2) >= MARGIN;
  };
  ...
  const pointWonBy = (name) => {
    ...
    if (player1.name === name) {
      ...
      if (tieBreak && player1Score >= TIE_BREAK_WIN && hasWonByMargin(player1Score, player2Score)) {
        player1.winGame();
        playerWon = true;
        player2.loseGame();
      } else if (!tieBreak && player1Score >= GAME_WIN && hasWonByMargin(player1Score, player2Score)) {
        player1.winGame();
        playerWon = true;
        player2.loseGame();
      }
    ...
    if (player2.name === name) {
      ...
      if (tieBreak && player2Score >= TIE_BREAK_WIN && hasWonByMargin(player1Score, player2Score)) {
        player2.winGame();
        playerWon = true;
        player1.loseGame();
      } else if (!tieBreak && player2Score >= GAME_WIN && hasWonByMargin(player1Score, player2Score)) {
        player2.winGame();
        playerWon = true;
        player1.loseGame();
      }
